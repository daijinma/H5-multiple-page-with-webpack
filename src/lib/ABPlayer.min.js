...
(function(){
	...
	PLP.create = function (element, params) {
		var elem = element;
		if(!params){
			params = {};
		}
		...
		var bind = ABP.bind(elem, params.mobile);
			...
			ABPInst.txtText.addEventListener("keyup", function(k){
				...
				if(k != null && k.keyCode === 13){
					...
					if(/^!/.test(this.value)){
						...
						switch (command){
							case "help":{
								var popup = ABPInst.createPopup("ๆ็คบไฟกๆฏ๏ผ",2000);
							}break;
							case "speed":
							case "rate":
							case "spd":{
								if(commandPrompts.length < 1){
									ABPInst.createPopup("้€ๅบฆ่ฐ่๏ผ่พ“ๅ…ฅ็พๅๆฏ”ใ€ 1% - 300% ใ€‘", 2000);
								}else{
							...
							case "volume":{
								if(commandPrompts.length == 0){
									var popup = ABPInst.createPopup("็ฎๅ้ณ้๏ผ" + 
									...
									ABPInst.createPopup("็ฎๅ้ณ้๏ผ" + 
										Math.round(ABPInst.video.volume * 100) + "%", 2000);
								}
							}break;
							default:break;
					...
					if(!k.shiftKey){
						...
						ABPInst.removePopup();
						var p = ABPInst.createPopup("็ฎๅ้ณ้๏ผ" + 
						...
						if(ABPInst.cmManager !== null){
							...
							ABPInst.removePopup();
							var p = ABPInst.createPopup("ๅผนๅน•้€ๆๅบฆ๏ผ" + Math.round(opa) + "%",800);
						}
					}
